...
//#include "cmep-payloads/dump_x/dump_x.h"
...
static int callsm_ex(k_root* kroot, int smctx, uint32_t pa) {
    ...
    int (*call_n_cry)(int smctx, int unk, uint32_t cmdpa) = KFUN("SceSblSsSmComm", 0xbb0);
    int (*world_economic_forum)(int a, int b, int c, int *d, int e) = KFUN("SceSblSsSmComm", 0xb10);
    if (!call_n_cry || !world_economic_forum) {
...
static int ussm_exec_1c(k_root* kroot, uint32_t ctx) {
    ...
    for (int i = 2; i < 0x3d0; i++) {
        sm_cmd->cargs.list.lv1[i].addr = (void*)0x0004a730;
    ...
    for (int i = 0x3a; i < 0x3d; i++)
        sm_cmd->cargs.list.lv1[i].addr = (void*)0x04b9da08;
...
static int cmep_init(int a0, int a1) {
    ...
    kroot->printf("[SNS][FUD] hello from cmep_init, getting funcs\n");
    
    // get funcs
    int (*load_ussm)(int* ctx, uint32_t * args) = KFUN("SceSblUpdateMgr", 0x42dc | 1);
    if (!load_ussm)
        return -1;

    kroot->printf("[SNS][FUD] loading update_sm\n");
    ...
    kroot->printf("[SNS][FUD] executing 1c\n");

    //kroot->memcpy(kroot->venezia_spram + 0x4000, dump_x_nmp, sizeof(dump_x_nmp));
