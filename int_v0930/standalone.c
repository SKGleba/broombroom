...
static int (*searchModule)(const char* name) = (void*)(0x2E1EB4 + 1);
static int (*getModule)(int pid, int uid, SceKernelModuleInfo* info) = (void*)(0x2E20E4 + 1);
...
int k_init(k_root* kroot) {
    void (*memset)(void* dst, int ch, int sz) = KFUN("SceSysmem", 0x2a24);
    if (!kroot || !memset)
        return -1;
    memset(kroot, 0, sizeof(k_root));
    kroot->memset = memset;
    kroot->memcpy = KFUN("SceSysmem", 0xd00);
    kroot->printf = KFUN("SceSysmem", 0x2a9b4 | 1);
    if (!kroot->memcpy || !kroot->printf)
        return -2;

    kroot->printf("[SNS][INIT] k_init started, allocing memblocks\n");

    int (*alloc_mb)(const char* name, int type, int size, alloc_mb_opt * opt) = KFUN("SceSysmem", 0x81c4 | 1);
    int (*get_mb)(int uid, void* out_va) = KFUN("SceSysmem", 0x19a9c | 1);

    kroot->printf("[SNS][INIT] alloc spad128k\n");
    alloc_mb_opt mbopt;
    ...
    kroot->printf("[SNS][INIT] tachyon edram\n");
    memset(&mbopt, 0, sizeof(alloc_mb_opt));
    mbopt.size = sizeof(alloc_mb_opt);
    mbopt.attr = 2;
    ...
    kroot->printf("[SNS][INIT] alloc pacDRAM\n");
    memset(&mbopt, 0, sizeof(alloc_mb_opt));
    mbopt.size = sizeof(alloc_mb_opt);
    mbopt.attr = 2;
    ...
    int (*umemcpy)() = KFUN("SceSysmem", 0x251f8 | 1);
...
void k_uinit(k_root* kroot) {
    ...
    int (*free_mb)(int uid) = KFUN("SceSysmem", 0x8b0c | 1);
    if (!free_mb)
        return;
...
reset_fud:
    ...
    kroot.printf("[SNS] finished without errors, NOT exiting\n");
